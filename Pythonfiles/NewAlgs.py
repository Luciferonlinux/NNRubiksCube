...
def main():
    oll_algos = pd.read_csv("../Datasets/oll_algos.csv", header=None, dtype=str).values
    pll_algos = pd.read_csv("../Datasets/pll_algos.csv", header=None, dtype=str).values
    ...
    # Generates the ID, Signature, Formula and one-hot-vector for PLL
    # teilweise von Pycubers CFOP-Solver kopiert und modifiziert
    ids = 0
    algo_dict = []
    for name, rec_id, algo in pll_algos:
        for i in range(4):
            ...
            for _ in range(4):
                print(len(algo_dict))
                print(algo_dict)
                if len(algo_dict) < 1 or smth not in algo_dict[0:][0:]:
    ...
    new_pll_algos.to_csv(
        path_or_buf="../Datasets/new_pll_algos.csv",
        sep=",",
        na_rep="",
    ...
    # ebenfalls teilweise von Pycubers CFOP-Solver kopiert und modifiziert
    ids = 0
    algo_dict = []
    ...
    new_pll_algos = pd.DataFrame(algo_dict, columns=('ID', 'Cube State', 'Formula', 'one-hot-vector'))
    new_pll_algos.to_csv(
        path_or_buf="../Datasets/new_oll_algos.csv",
        sep=",",
        na_rep="",
        columns=('ID', 'Cube State', 'Formula', 'one-hot-vector'),
        index=False
