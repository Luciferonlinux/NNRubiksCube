...
def LL_scramble(count, path, write=True, processes=16):
    ...
    def pretty_loading_animation():
        ...
        if done:
            stop = time.perf_counter()
            stdout.write(f"\x1b[2K\rDone in {stop - start:.2f}s\nDataset is saved in {datasetpath}")
...
if __name__ == '__main__':
    ...
    confirm = input("You are about to create a Dataset\n"
                    "This will use up all available CPU resources and consequently Lag your System\n"
                    "Do you still wish to continue?\n"
                    "Type Y to continue or N to abort\n").lower()
    if confirm == "y":
        count = int(input("How Many samples do you whish to generate?\n"))
        start = time.perf_counter()
        LL_scramble(count, datasetpath, write=False)
